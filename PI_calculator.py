...
def solve_for_z1_z2_given_z3_and_ratio(z3_total, target_pi, target_f1_ratio=0.8):
    ...
    """
    k = (10**target_pi) / 100
    denom = target_f1_ratio + k
    if denom == 0: return 0, 0
    
    f3_ideal = (k * (1 - target_f1_ratio)) / denom
    
    if f3_ideal <= 0.0001: return 0, 0 
    ...
    return z1_ideal, max(0, z2_ideal)
...
with tab3:
    ...
    calc_note = "" # Inizializzazione sicura
    ...
    if z1_ideal >= m1 and z2_ideal >= m2:
        ...
        # Scenario Complesso
        # Caso A: Troppa Z1 già fatta. Fissiamo Z1=m1 e risolviamo per Z2.
        ...
        if z2_calc >= m2:
            ...
            # Caso B: Troppa Z2 già fatta. Dobbiamo aumentare Z1 per diluire.
            K = (10**target_pi_val) / 100.0
            ...
            if denom > 0:
                ...
                st.error("⚠️ Configurazione matematica impossibile con questi minuti eseguiti. Prova ad aumentare Z3.")
                z1_final, z2_final, z3_final = m1, m2, m3
                calc_note = "Errore: Soluzione non trovata"
